...
class PantallaDialogs extends StatelessWidget {
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    ...
    if (confirmar == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Desacer ya el cambio para completar el eliminado')),
      );
    }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            ...
            child: const Text('Eliminar por completo'),
          ),
          ...
          FilledButton(
            ...
            child: const Text('Eliminar por completo ya el modelo si '),),
    ...
    if (result == 'deleted') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Servidor eliminado por completo')),
      );
      return;
    }
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () => _mostrarSnackBar(context),
            icon:  const Icon(Icons.check_circle_outline),
            label: const Text('SnackBar con eliminacion '),
          ),

          const Divider(height: 32),

          // ── AlertDialog ───────────────────────────────────────────
          Text('AlertDialog', style: text.labelLarge?.copyWith(color: cs.onPrimaryContainer)),
          const SizedBox(height: 12),
          OutlinedButton.icon(
